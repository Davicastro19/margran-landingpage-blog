		...
		while True:
			try:
				if self.get_async_order(id_number)["option-closed"]!={}:
					result = self.get_async_order(id_number)["option-closed"]
					break
			except Exception as a:
				pass
		return result["msg"]["profit_amount"]-result["msg"]["amount"],result["msg"]["open_time"],result["msg"]["expiration_time"],result["msg"]["profit_percent"] 
		...
		if order_data != None:
			if order_data["status"] == "closed":
				if order_data["close_reason"] == "expired":
					return True,order_data["close_profit"]- order_data["invest"],order_data["open_time"],order_data["close_time"]
				elif order_data["close_reason"] == "default":
					return True, order_data["pnl_realized"],order_data["open_time"],order_data["close_time"]
			else:
				return False, None,"",""
		else:
			return False, None,"",""
